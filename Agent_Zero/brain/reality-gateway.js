...
export function buildGatewayPrompt({ stateFile, factSheetFiles = [], markers, runId, learningsBlock = renderBrainLearningsBlock().markdown }) {
  ...
  return [
    ...
    '- If the run discovered or referenced PDF, DOCX, XLSX, or other source attachments, did it ask targeted WorkIQ questions about the attachment, cite concrete attachment-derived facts, and mark the ledger yes(workiq-index); or did it fail closed with failed(<reason>)?',
