...
const CHAT_MARKER_GRAMMAR = [
  ...
  '[LINEITEM_NEW] {"taskId":"task-...","sourceRefs":[],"lineItem":{"id":"li-...","title":"...","category":"action","priority":"critical|high|medium|low","status":"open","owner":"user","userActionRequired":true,"userAction":"...","currentState":"...","plannedNext":null,"dueAt":null,"waitingOn":null,"problem":null,"risk":null,"confidence":"medium","relevance":{"score":82,"reason":"Project-level consequence in plain English.","evidenceRefIds":["src-..."],"assessedAt":"..."},"evidenceRefIds":["src-..."],"state":"confirmed","sources":[],"lastConfirmedByMessageDate":"...","threadRef":"conversation-id","lastVerifiedMessageDate":"...","resolutionStatus":"open","askQuote":{"text":"...","from":"...","date":"...","threadRef":"conversation-id"},"threadCheck":{"coverage":"complete","addressedTo":"user","messageCount":1,"lastMessageDate":"...","checkedThroughMessageDate":"..."}},"processingLedger":[{"itemRef":{"type":"email","id":"..."},"threadRef":"conversation-id","date":"...","disposition":"new-node","nodeRefs":["li-..."],"attachmentsHandled":"none","quote":"short verbatim quote","reason":"why this disposition is correct"}]}',
  '[LINEITEM_UPDATE] {"taskId":"task-...","lineItemId":"li-...","sourceRefs":[],"patch":{"status":"waiting","currentState":"...","confidence":"medium","relevance":{"score":82,"reason":"Project-level consequence in plain English.","evidenceRefIds":["src-..."],"assessedAt":"..."},"state":"confirmed","sources":[],"lastConfirmedByMessageDate":"..."},"processingLedger":[{"itemRef":{"type":"email","id":"..."},"threadRef":"conversation-id","date":"...","disposition":"updates-node","nodeRefs":["li-..."],"attachmentsHandled":"failed(<reason>)","quote":"short verbatim quote","reason":"why this disposition is correct"}],"evidenceRefIds":["src-..."]}',
...
export function buildTaskChatDeepVerifyPrompt({
  ...
  return [
    ...
    '- Keep project summary at most 420 characters and pmStatus.current at most 520 characters. Put owner, next step, due date, waiting dependency, problem, risk, and compatibility priority in typed line-item fields instead of duplicating chronology.',
    '- Reassess every active project line item whenever evidence changes. Store semantic relevance as {score:0..100,reason,evidenceRefIds,assessedAt}; compare consequence, time sensitivity, escalation, user obligation, blocker/dependency scope, reversibility, and freshness across the whole project. Do not count keywords or let confidence/review status change the score.',
    ...
    '- LEARNING markers may only propose reusable, general operating memory. They must not contain task facts, secrets, credentials, or one-off project state.',
    '- Every status, problem, risk, waiting, or user-action change needs evidenceRefIds.',
    '- Every confirmed active line item needs semantic relevance with a plain-English project-level reason and evidenceRefIds. Any new source id must be included in sourceRefs on that same mutation marker.',
    '- Temporal obsolete/superseded bookings for stale dates must use standalone NODE_OBSOLETE markers, never a bundled PROJECT_UPDATE.pmStatus or LINEITEM_UPDATE.',
    '- Unsupported or uncertain changes should be NEEDS_REVIEW, not asserted as state.',
    '',
