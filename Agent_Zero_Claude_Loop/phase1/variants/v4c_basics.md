...
**For Teams messages:** only include if there is a clear, specific request directed at the user with a concrete deliverable. Teams chats are conversational — most do not require action.
...
### Teams-Specific: Always Skip

- Short conversational snippets ("ok sounds good", "👍", "joining now")
- Casual or rhetorical questions with no deliverable
- Status or scheduling messages ("I'll be 5 min late")
...
5. **No invented IDs** — the `existingId` field must contain an ID that exists verbatim in the provided task list. Never guess, fabricate, or construct an ID. If unsure about a match, classify as "new" instead of risking a wrong update.
